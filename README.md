...
### Using Background Audio in GUI

В GUI можно выбрать опциональный фоновый аудиотрек в поле **"Background audio (optional):"** через кнопку **"Browse..."**.

Что происходит дальше:
- если файл указан, путь сохраняется в конфиг запуска и передаётся в pipeline как `background_track`;
- pipeline выполняет смешивание фоновой дорожки с переведённым голосом на шаге аудио-микширования;
- если путь не выбран, поведение остаётся прежним (используется только дорожка переведённого голоса).

Важно: это опциональная настройка, без выбора фоновой дорожки пайплайн работает как раньше.
